...
def check_username(username):
    instagram_username_pattern = bool(
        re.match(r'^([A-Za-z0-9_](?:(?:[A-Za-z0-9_]|(?:\.(?!\.))){0,28}(?:[A-Za-z0-9_]))?)$', username))
...
class LoginUserSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if not user:
            raise serializers.ValidationError({'message': 'Account with username does not exists.'},
...
class UpdateUserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ('username', 'email_id', 'profile_picture', 'name', 'about_me', 'phone_number', 'email')
        # extra_kwargs = {
